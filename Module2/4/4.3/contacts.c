...
static contact_node_t* tree_get_node_by_index_rec(contact_node_t *node, size_t target, size_t *curr) {
    if (!node) return NULL;
    
    contact_node_t *left_res = tree_get_node_by_index_rec(node->left, target, curr);
    ...
    return tree_get_node_by_index_rec(node->right, target, curr);
}

static contact_node_t* contacts_get_node_at(const contacts_t *self, size_t index) {
    if (!self || index >= self->size) return NULL;
    size_t start_index = 0;
    return tree_get_node_by_index_rec(self->root, index, &start_index);
}

static void tree_free_rec(contact_node_t *node) {
    if (!node) return;
    tree_free_rec(node->left);
    tree_free_rec(node->right);
    record_free(&node->record);
    free(node);
}

static void tree_flatten(contact_node_t *node, contact_node_t **arr, size_t *index) {
    if (!node) return;
    tree_flatten(node->left, arr, index);
    arr[*index] = node;
    (*index)++;
    tree_flatten(node->right, arr, index);
}

static contact_node_t* tree_rebuild_balanced(contact_node_t **arr, int start, int end) {
    if (start > end) return NULL;
    
    int mid = start + (end - start) / 2;
    contact_node_t *root = arr[mid];
    
    root->left = tree_rebuild_balanced(arr, start, mid - 1);
    root->right = tree_rebuild_balanced(arr, mid + 1, end);
    
    return root;
}

void contacts_balance(contacts_t *self) {
    if (!self || self->size <= 2) return;
    
    contact_node_t **arr = malloc(self->size * sizeof(contact_node_t*));
    if (!arr) return;
    
    size_t index = 0;
    tree_flatten(self->root, arr, &index);
    self->root = tree_rebuild_balanced(arr, 0, (int)self->size - 1);
    
    free(arr);
    self->mod_count = 0;
}

static contact_node_t* tree_insert(contact_node_t *root, contact_node_t *new_node) {
    if (!root) return new_node;
    
    int cmp = record_compare(&new_node->record, &root->record);
    if (cmp < 0) {
...
static contact_node_t* tree_delete(contact_node_t *root, contact_node_t *target, int *deleted) {
    if (!root) return NULL;
    
    int cmp = record_compare(&target->record, &root->record);
    if (cmp < 0) {
        root->left = tree_delete(root->left, target, deleted);
    } else if (cmp > 0) {
        root->right = tree_delete(root->right, target, deleted);
    } else {
        if (root != target) {
            root->right = tree_delete(root->right, target, deleted);
        } else {
            *deleted = 1;
            if (!root->left) {
                contact_node_t *temp = root->right;
                record_free(&root->record);
                free(root);
                return temp;
            } else if (!root->right) {
                contact_node_t *temp = root->left;
                record_free(&root->record);
                free(root);
                return temp;
            }
            ...
            record_free(&root->record);
            record_init(&root->record);
            record_copy(&root->record, &temp->record);
            
            root->right = tree_delete(root->right, temp, deleted);
        }
    }
    return root;
}
...
contacts_status_t contacts_add(contacts_t *self, const record_t *rec) {
    ...
    self->root = tree_insert(self->root, new_node);
    self->size++;
    self->mod_count++;

    if (self->mod_count >= self->balance_threshold) {
        contacts_balance(self);
    }
...
contacts_status_t contacts_update_at(contacts_t *self, size_t index, const record_t *new_rec) {
    ...
    int deleted = 0;
    self->root = tree_delete(self->root, node, &deleted);
    self->size--;
    ...
    self->root = tree_insert(self->root, node);
    self->size++;
    self->mod_count++;
